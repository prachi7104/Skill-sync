...
export default function AdminStudentsPage() {
  ...
  // Fetch students based on search/filter
  const performSearch = useCallback(
    async (q: string, branch: string, year: string, page: number) => {
      ...
      try {
        ...
        const res = await fetch(`/api/admin/students/search?${params}`);
        ...
        // Extract unique branches and batch years from results
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-slate-900 border border-white/5 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="px-8 py-6 space-y-8">
              ...
              <div className="space-y-2">
                {student.achievements.map((ach, i) => (
                  <div key={i} className="bg-slate-950/50 border border-slate-800 rounded-xl p-3">
                    ...
                    <p className="text-xs text-slate-500 mt-1">{ach.date}</p>
                  </div>
                ))}
              </div>
            </section>
          ...
          {/* AI Status */}
          <section>
            <h3 className="text-lg font-bold text-white mb-4">AI Status</h3>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-3">
                <p className="text-slate-400">Has Embedding</p>
                <p className={`font-semibold ${student.hasEmbedding ? "text-emerald-400" : "text-amber-400"}`}>
                  {student.hasEmbedding ? "✓ Yes" : "⏳ Pending"}
                </p>
              </div>
              <div className="bg-slate-950/50 border border-slate-800 rounded-xl p-3">
                <p className="text-slate-400">Profile Completeness</p>
                <p className="text-white font-semibold">{student.profileCompleteness}%</p>
              </div>
            </div>
          </section>
        </div>
