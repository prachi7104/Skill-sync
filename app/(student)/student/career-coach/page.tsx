...
function timeAgoLabel(iso: string | undefined, cached: boolean): string {
  ...
  if (mins < 60) {
    return cached
      ? `Cached from ${mins} minutes ago`
      : `Generated ${mins} minutes ago`;
  }
  const hours = Math.floor(mins / 60);
  return cached
    ? `Cached from ${hours} hours ago`
    : `Generated ${hours} hours ago`;
}
  ...
  const fetchRoadmap = useCallback(async (refresh = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        setPayload(null);
        setErrorText(
          stripMarkdown(data.error || "Unable to generate roadmap right now."),
        );
        return;
      }
      ...
      if (data.message || data.error || !data.priority_skills?.length) {
        setPayload(data);
        setErrorText(
          stripMarkdown(data.message || data.error || "No drives to analyze"),
        );
        return;
      }
      ...
      setPayload({
        ...
        priority_skills: (data.priority_skills || []).map((skill) => ({
          ...
          resource: {
            ...
            url_description: stripMarkdown(
              skill.resource?.url_description || "",
            ),
          },
        })),
      });
  ...
  async function sendMessage(rawMessage?: string) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Network issue while contacting Career Advisor.",
        },
      ]);
    } finally {
        ...
        <header className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-muted-foreground">
              Focus on the skills, resources, and AMCAT signals that move you
              closer to eligible drives.
            </p>
          ...
          <Button
            type="button"
            variant="outline"
            className="border-border bg-card text-foreground hover:bg-muted/80"
          ...
          <Card className="border border-warning/20 bg-warning/10 dark:bg-warning/10">
            <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
              ...
              <h2 className="text-xl font-bold text-warning">
                No drives to analyze
              </h2>
              <p className="max-w-2xl text-sm text-warning/90">
                {payload?.suggestion ||
                  "Complete your profile and wait for active drives to be posted"}
              </p>
              <p className="text-xs text-warning/80">{errorText}</p>
              <Button
                type="button"
                className="mt-2 bg-warning/10 text-foreground hover:bg-warning/10"
                onClick={() => void fetchRoadmap(true)}
              >
                Retry
              </Button>
            ...
            {skills.map((skill, index) => (
              ...
              >
                <CardHeader className="space-y-3">
                  ...
                  <span className="inline-flex w-fit items-center rounded-full border border-border bg-muted px-2.5 py-1 text-xs text-muted-foreground">
                    Start Week {skill.week_start ?? "?"} ·{" "}
                    {skill.hours_needed ?? "?"} hours
                  </span>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="rounded-xl border border-border bg-muted p-3">
                    <div className="mb-2 flex items-center justify-between">
                      <span
                        className={`rounded-full border px-2.5 py-0.5 text-xs font-semibold ${resourceTypeClass(skill.resource?.type)}`}
                      >
                        {skill.resource?.type || "Resource"}
                      </span>
                    </div>
                    <p className="text-sm font-bold text-foreground">
                      {skill.resource?.name || "Recommended Resource"}
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground">
                      Search:{" "}
                      {skill.resource?.url_description ||
                        "Find an official resource"}
                    </p>
                  </div>
                </CardContent>
              </Card>
