...
// //     user: StudentUser;
// //     profile: any; // We use any here as a base, but form values are strictly typed via StudentProfileInput
// // }
...
// //     return (
// //         <div className="max-w-7xl mx-auto space-y-8 p-6 md:p-10 animate-in fade-in duration-700">
...
// //                                         </FormItem>
// //                                     )} />
// //                                     {/* Additional Academic Fields mapped to form values */}
// //                                 </div>
// //                             ) : (
// //                                 <div className="grid grid-cols-2 gap-3">
// //                                     <MetaField label="Roll No" value={profile.rollNo} />
...
// //                             )}
// //                         </div>
// //                         {/* Remaining Left Rail components strictly typed... */}
// //                     </div>

// //                     {/* RIGHT RAIL */}
// //                     <div className="lg:col-span-8 space-y-8">
// //                         {/* Resume Section with Type-Safe Upload */}
// //                         <div className="bg-slate-900 rounded-[2rem] border border-white/5 p-7 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
// //                             <div className="flex items-center gap-4">
// //                                 <div className="p-3.5 bg-indigo-500/10 rounded-2xl"><FileText className="w-6 h-6 text-indigo-400" /></div>
// //                                 <div>
...
// //                         {/* Experience Timeline Section with Strict Typing */}
// //                         <div className="bg-slate-900/50 rounded-[2rem] border border-white/5 p-8">
// //                             <div className="flex items-center justify-between mb-10">
// //                                 <div className="flex items-center gap-3">
// //                                     <div className="p-2.5 bg-amber-500/10 rounded-xl"><Briefcase className="w-5 h-5 text-amber-400" /></div>
...
// //                                             <div className="space-y-2">
// //                                                 <h3 className="text-lg font-bold text-white">{exp.role}</h3>
// //                                                 <p className="text-amber-400 font-semibold text-sm">{exp.company}</p>
// //                                                 <p className="text-slate-400 text-sm leading-relaxed max-w-2xl">{exp.description}</p>
// //                                             </div>
// //                                         </div>
// //                                     )) : <div className="text-slate-500 text-sm">No experience listed.</div>}
// //                                 </div>
// //                             )}
// //                         </div>
// //                     </div>
...
// import { zodResolver } from "@hookform/resolvers/zod";
// import { 
//     Briefcase, Code2, GraduationCap, Link2, Mail, User, 
//     Loader2, Trash2, Save, FileText, Upload, Sparkles, 
//     ExternalLink
// } from "lucide-react";
...
// interface MetaFieldProps { label: string; value: string | number | null | undefined; highlight?: boolean; }
...
//     return (
//         <div className="max-w-7xl mx-auto space-y-8 p-6 md:p-10 animate-in fade-in duration-700 pb-32">
//             <div className="bg-slate-900 rounded-[2.5rem] border border-white/5 p-8 relative overflow-hidden">
//                 <div className="flex flex-col lg:flex-row gap-8 items-center justify-between relative z-10">
//                     <div className="flex items-center gap-6">
//                         <div className="w-24 h-24 rounded-3xl bg-gradient-to-br from-indigo-500 to-violet-700 flex items-center justify-center text-3xl font-black text-white">{initials}</div>
//                         <div><h1 className="text-3xl font-black text-white tracking-tight">{user.name}</h1><p className="flex items-center gap-2 text-slate-400 font-medium text-sm"><Mail className="w-4 h-4" /> {user.email}</p></div>
//                     </div>
//                     <div className="flex items-center gap-6">
//                         <div className="bg-slate-950/50 p-4 rounded-3xl border border-white/5 min-w-[200px] text-center">
//                             <span className="text-[10px] font-black uppercase text-slate-500 flex items-center justify-center gap-2"><Sparkles className="w-3 h-3 text-indigo-400" /> Identity Score</span>
//                             <span className="text-xl font-black text-white">{score}%</span>
//                         </div>
//                         <button onClick={() => isEditing ? form.handleSubmit(onSubmit)() : setIsEditing(true)} className="bg-white text-slate-950 px-8 py-3 rounded-2xl font-bold text-sm">
//                             {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : isEditing ? <Save className="w-4 h-4" /> : <User className="w-4 h-4" />} {isEditing ? "Save" : "Edit Profile"}
//                         </button>
//                     </div>
//                 </div>
//             </div>
...
//                     <div className="lg:col-span-4 space-y-8">
//                         <div className="bg-slate-900/50 rounded-[2rem] border border-white/5 p-7">
//                             <h2 className="font-bold text-white text-lg mb-6 flex items-center gap-3"><GraduationCap className="w-5 h-5 text-blue-400" /> Academic Roots</h2>
//                             {isEditing ? (
//                                 <div className="space-y-4">
//                                     <FormField control={form.control} name="rollNo" render={({ field }) => (
//                                         <FormItem><FormLabel className="text-xs text-slate-500 uppercase font-bold">Roll No</FormLabel><FormControl><Input className={inputClass} {...field} value={field.value ?? ""} /></FormControl></FormItem>
//                                     )} />
//                                     <FormField control={form.control} name="cgpa" render={({ field }) => (
//                                         <FormItem><FormLabel className="text-xs text-slate-500 uppercase font-bold">CGPA</FormLabel><FormControl><Input type="number" step="0.01" className={inputClass} {...field} value={field.value ?? ""} onChange={(e) => field.onChange(parseFloat(e.target.value))} /></FormControl></FormItem>
//                                     )} />
//                                 </div>
//                             ) : (
//                                 <div className="grid grid-cols-2 gap-3">
...
//                         <div className="bg-slate-900/50 rounded-[2rem] border border-white/5 p-7">
//                             <h2 className="font-bold text-white text-lg mb-6 flex items-center gap-3"><Code2 className="w-5 h-5 text-emerald-400" /> Skills</h2>
//                             <div className="flex flex-wrap gap-2">
//                                 {isEditing ? skillFields.map((f, i) => (
//                                     <div key={f.id} className="flex gap-2 w-full">
//                                         <FormField control={form.control} name={`skills.${i}.name`} render={({ field }) => <FormControl><Input className={inputClass} {...field} /></FormControl>} />
//                                         <button onClick={() => removeSkill(i)} className="p-3 text-rose-500"><Trash2 className="w-4 h-4" /></button>
//                                     </div>
//                                 )) : profile.skills?.map((s: any, i: number) => <span key={i} className="px-3 py-1.5 bg-slate-950 border border-slate-800 text-slate-300 text-xs font-bold rounded-lg">{s.name}</span>)}
//                             </div>
//                         </div>
//                     </div>

//                     <div className="lg:col-span-8 space-y-8">
//                         <div className="bg-slate-900 rounded-[2.5rem] border border-white/5 p-7 flex items-center justify-between">
//                             <div className="flex items-center gap-4"><div className="p-3 bg-indigo-500/10 rounded-xl"><FileText className="w-5 h-5 text-indigo-400" /></div><h2 className="text-white font-bold">Master Resume</h2></div>
//                             <div className="relative">
//                                 <Input type="file" accept=".pdf" onChange={handleFileUpload} className="absolute inset-0 opacity-0 cursor-pointer" />
//                                 <button className="bg-indigo-600 text-white px-6 py-2 rounded-xl text-sm font-bold flex items-center gap-2">{isUploading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Upload className="w-4 h-4" />} Upload PDF</button>
//                             </div>
//                         </div>

//                         <div className="bg-slate-900/50 rounded-[2.5rem] border border-white/5 p-8">
//                             <h2 className="text-white font-bold text-xl mb-8 flex items-center gap-3"><Briefcase className="w-5 h-5 text-amber-400" /> Experience</h2>
//                             {isEditing ? expFields.map((field, idx) => (
//                                 <div key={field.id} className="p-6 bg-slate-950/50 border border-slate-800 rounded-2xl mb-4 relative">
//                                     <button onClick={() => removeExp(idx)} className="absolute top-4 right-4 text-rose-500"><Trash2 className="w-4 h-4" /></button>
//                                     <FormField control={form.control} name={`workExperience.${idx}.role`} render={({field}) => <Input className={inputClass} placeholder="Role" {...field} value={field.value ?? ""} />} />
//                                 </div>
//                             )) : profile.workExperience?.map((exp: any, i: number) => (
//                                 <div key={i} className="relative pl-10 border-l border-slate-800 pb-8 last:pb-0">
//                                     <div className="absolute left-[-5px] top-1 w-2.5 h-2.5 bg-amber-500 rounded-full" />
...
//             <p className="text-[9px] font-black text-slate-500 uppercase mb-1.5">{label}</p>
//             <p className={`text-sm font-bold ${highlight ? 'text-indigo-400' : 'text-slate-200'}`}>{value ?? "—"}</p>
//         </div>
//     );
// }

// function EmptyState({ message }: { message: string }) {
//     return <div className="py-8 border-2 border-dashed border-slate-800 rounded-2xl text-center text-slate-500 font-bold">{message}</div>;
