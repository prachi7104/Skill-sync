...
function simulateCreateFacultyPayload(input: {
  email?: string;
  name?: string;
  password?: string;
}) {
  if (!input.email || !input.name) {
    return { ok: false, status: 400, error: "Missing required fields: email and name are required" };
  }

  const candidatePassword = input.password || "AutoGeneratedA1!";
  const validation = validatePasswordStrength(candidatePassword);
  if (!validation.valid) {
    return { ok: false, status: 400, error: validation.reason };
  }

  return { ok: true, status: 201, generatedPassword: candidatePassword };
}
...
describe("API Routes", () => {
  ...
  describe("POST /api/admin/faculty - auto password behavior", () => {
    it("should allow missing password and auto-generate one", () => {
      const result = simulateCreateFacultyPayload({
    ...
    it("should reject weak provided password", () => {
      const result = simulateCreateFacultyPayload({
        email: "faculty@example.com",
        name: "Dr Faculty",
        password: "weak",
  ...
  describe("POST /api/auth/change-password", () => {
    it("should block students", () => {
      const result = simulateChangePassword("student", "old", "NewStrong1");
      expect(result.ok).toBe(false);
      expect(result.status).toBe(400);
    });

    it("should require both fields for staff", () => {
      const result = simulateChangePassword("faculty", "old");
      expect(result.ok).toBe(false);
      expect(result.status).toBe(400);
    });

    it("should enforce password strength for staff", () => {
      const result = simulateChangePassword("admin", "old", "weakpass");
      expect(result.ok).toBe(false);
      expect(result.status).toBe(400);
    });

    it("should accept valid staff payload", () => {
      const result = simulateChangePassword("faculty", "OldPass1", "NewPass123");
      expect(result.ok).toBe(true);
