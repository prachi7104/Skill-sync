...
type LockedField = "batchYear" | "sapId";

function validateLockedField(current: unknown, incoming: unknown, field: LockedField): { ok: boolean; status: number } {
...
describe("P1.3 profile patch validation", () => {
  ...
  it("blocks changing locked sapId once set", () => {
    const result = validateLockedField("500126666", "500126667", "sapId");
    expect(result.ok).toBe(false);
    expect(result.status).toBe(400);
  });

  it("allows first-time sapId assignment", () => {
    const result = validateLockedField(null, "500126666", "sapId");
    expect(result.ok).toBe(true);
    expect(result.status).toBe(200);
  });

  it("blocks changing batchYear once set", () => {
    const result = validateLockedField(2027, 2028, "batchYear");
    expect(result.ok).toBe(false);
    expect(result.status).toBe(400);
  });
});
