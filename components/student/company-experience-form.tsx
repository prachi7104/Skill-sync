    ...
    <div className="mx-auto grid max-w-6xl gap-6 p-8 lg:grid-cols-[1.2fr_0.8fr]">
      <Card>
        ...
        <CardContent className="space-y-5">
          <div className="space-y-2">
            ...
            <Input value={companyName} onChange={(event) => setCompanyName(event.target.value)} list="company-suggestions" />
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <select value={driveType} onChange={(event) => setDriveType(event.target.value)} className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm text-foreground">
          ...
          <div className="space-y-2">
            <Label>Outcome</Label>
            <select value={outcome} onChange={(event) => setOutcome(event.target.value)} className="h-10 w-full rounded-md border border-border bg-background px-3 text-sm text-foreground">
          ...
          <div className="space-y-2">
            ...
            <Textarea rows={8} maxLength={1500} value={interviewProcess} onChange={(event) => setInterviewProcess(event.target.value)} />
            <p className="text-xs text-muted-foreground">{interviewProcess.length}/1500</p>
          </div>
          <div className="space-y-2">
            <Label>Tips for Future Students</Label>
            <Textarea rows={6} maxLength={1000} value={tips} onChange={(event) => setTips(event.target.value)} />
            ...
            <div className="flex items-center gap-2">
              {[1, 2, 3, 4, 5].map((value) => (
                <button key={value} type="button" onClick={() => setDifficulty(value)}>
                  <Star className={cn("h-6 w-6", value <= difficulty ? "fill-amber-400 text-amber-400" : "text-muted-foreground")} />
                </button>
              ))}
            </div>
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <label className="flex items-center gap-3 rounded-md border border-border bg-background px-4 py-3 text-sm text-foreground">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {tips ? (
            <div>
              <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Tips</h4>
