...
export function DriveConflictsButton() {
  ...
  return (
    <>
      <Button variant="outline" className="border-border bg-card text-foreground hover:bg-secondary" onClick={loadConflicts}>
        Check Conflicts
      </Button>
        ...
        <DialogContent className="max-w-3xl border-border bg-background text-foreground">
          ...
          <div className="space-y-3">
            ...
            {!loading ? conflicts.map((conflict) => (
              <div key={`${conflict.drive1.id}-${conflict.drive2.id}`} className="rounded-md border border-amber-500/20 bg-amber-500/5 p-4">
